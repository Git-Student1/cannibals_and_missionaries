...
async function start_animation() {
    ...
    function createDrawnPeople(draw, x_center, y_center, diameter, n, color, is_cannibal){
        ...
        const elements = []
        for (let i = 0; i < n; i++) {
            elements[elements.length] = createDrawnPerson(draw, diameter, is_cannibal,  color );
        }
        return elements
    }
    ...
    const people_size = (boat_width/(n_cannibals+n_missionaries));

    const cannibals_start = createDrawnPeople(draw, start_beach_properties.center_x(), start_beach_properties.center_y(), people_size, n_cannibals, "red", true)
    const missionaries_start =  createDrawnPeople(draw, start_beach_properties.center_x()+people_size*1.2, start_beach_properties.center_y(), people_size, n_missionaries, "blue", false)
